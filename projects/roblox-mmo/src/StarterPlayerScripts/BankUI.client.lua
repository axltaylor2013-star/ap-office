...
titleLabel.Text = "🏦 BANK OF WILDERNESS"
...
local depositAllBtn = createButton("📥 Deposit All", COLORS.green, buttonBar)
local closeBottomBtn = createButton("✕ Close Bank", COLORS.red, buttonBar)
...
local function updateBankDisplay(bankData, invData)
	...
	-- Update bank items
	bankTitle.Text = "🏦 BANK (" .. #bankData .. "/100)"
	...
	invTitle.Text = "🎒 INVENTORY (" .. #invData .. "/28)"
		...
		bankUpdateRemote.OnClientEvent:Connect(function(bankData, invData, success, action)
			...
			if success then
				if action == "deposit" then
					showStatus("✅ Item deposited successfully!", COLORS.green)
				elseif action == "withdraw" then
					showStatus("✅ Item withdrawn successfully!", COLORS.green)
				elseif action == "deposit_all" then
					showStatus("✅ All items deposited!", COLORS.green)
				end
			else
				if action == "deposit" or action == "deposit_all" then
					showStatus("❌ Bank is full or item not found!", COLORS.red)
				elseif action == "withdraw" then
					showStatus("❌ Inventory full or item not found!", COLORS.red)
				end
			end
		end)
	end
